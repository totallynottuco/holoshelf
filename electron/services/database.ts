...
import {
  ...
  getHololiveCanonicalTalentId,
  getHololiveExpandedTalentIds
} from "../../src/shared/hololiveTalentIdentity";
import type {
  HololiveMusicLibraryCollabScope,
  HololiveMusicLibrarySort,
  HololiveCustomSongUpsertRequest,
...
import {
  ...
  hasHololiveLowPriorityLiveVersionMarker,
...
import { parseYouTubeVideoId } from "./youtubeVideoStatsService";
import {
...
const HOLOLIVE_BRACKET_VOCAL_SCOPES: HololiveBracketVocalScope[] = ["solo", "group"];
const HOLOLIVE_BRACKET_TALENT_STATUS_FILTERS: HololiveBracketTalentStatusFilter[] = ["active", "alumni", "custom"];
const HOLOLIVE_BRACKET_HISTORY_PARTICIPATION_VALUES: HololiveBracketHistoryParticipation[] = [
  "never",
  "appeared",
  "top8",
  "top4",
...
const CURRENT_HOLOLIVE_CARD_CACHE_SUFFIX = `-card-v${HOLOLIVE_CARD_IMAGE_CACHE_VERSION}.png`;
...
interface HololiveBracketSelectionOptions {
  maxEntriesPerTalent: number | null;
  preferTopicSplitPerTalent: boolean;
}
...
interface HololiveBracketHistoryExclusions {
  ...
  participation: {
    mode: HololiveBracketHistoryParticipation;
    youtubeVideoIds: Set<string>;
    canonicalPerformanceKeys: Set<string>;
  } | null;
}

interface HololiveBracketGenerationResult {
  entries: HololiveBracketEntry[];
  warnings: HololiveBracketCreateWarning[];
}

interface HololiveBracketCandidateSelectionResult {
...
const MIGRATIONS = [
  ...
  },
  {
    version: 29,
    sql: `
      ALTER TABLE hololive_music_videos
        ADD COLUMN source_kind TEXT NOT NULL DEFAULT 'official'
          CHECK(source_kind IN ('official', 'user'));
      ...
      WHERE EXISTS (
        ...
        WHERE i.source = 'custom'
          AND (
            i.id = hololive_music_videos.idol_id
            OR EXISTS (SELECT 1 FROM json_each(hololive_music_videos.participant_idol_ids_json) p WHERE p.value = i.id)
            OR EXISTS (SELECT 1 FROM json_each(hololive_music_videos.owned_idol_ids_json) o WHERE o.value = i.id)
            OR EXISTS (SELECT 1 FROM json_each(hololive_music_videos.featured_idol_ids_json) f WHERE f.value = i.id)
          )
      );

      CREATE INDEX IF NOT EXISTS idx_hololive_music_videos_source_kind
        ON hololive_music_videos(source_kind);
    `
  },
  {
    version: 30,
    sql: `
      ALTER TABLE hololive_idols ADD COLUMN card_image_url TEXT;

      CREATE TABLE hololive_image_cache_v30 (
        idol_id TEXT NOT NULL REFERENCES hololive_idols(id) ON DELETE CASCADE,
        kind TEXT NOT NULL CHECK(kind IN ('icon', 'profile', 'card')),
        source_url TEXT NOT NULL,
        local_filename TEXT NOT NULL,
        mime_type TEXT,
        size_bytes INTEGER,
      ...
      INSERT INTO hololive_image_cache_v30
        (idol_id, kind, source_url, local_filename, mime_type, size_bytes, updated_at)
      SELECT idol_id, kind, source_url, local_filename, mime_type, size_bytes, updated_at
      FROM hololive_image_cache
      WHERE kind IN ('icon', 'profile', 'card');

      DROP TABLE hololive_image_cache;
      ALTER TABLE hololive_image_cache_v30 RENAME TO hololive_image_cache;
    `
  },
  {
    version: 31,
    sql: `
      ALTER TABLE hololive_music_videos
        ADD COLUMN official_catalog_active INTEGER NOT NULL DEFAULT 1
          CHECK(official_catalog_active IN (0, 1));
...
export class DatabaseService {
      ...
      this.run(
        `INSERT INTO hololive_idols
           (id, slug, display_name, branch, generation, status, source, official_url, icon_url,
            profile_image_url, card_image_url, profile_quote, youtube_channel_url, youtube_channel_id, x_handle, x_url,
  ...
  repairHololiveMusicClassifications(): void {
    this.backfillHololiveMusicClassifications();
  }
  ...
  repairHololiveMusicDuplicateRows(runId = "repair"): number {
    ...
    const repair = () => {
      ...
      repairedRows += this.purgeCanonicalVariantDuplicateRows(runId);
      repairedRows += this.purgeSupersededDirectUploadRows(runId);
      if (repairedRows > 0) {
  ...
  importHolodexMusicArtifacts(
    ...
    this.transaction(() => {
      ...
      canonicalDuplicateCount += this.purgeSupersededDirectUploadRows(runId);
  ...
  clearHolodexMusicData(): void {
    const itemRows = this.select<{ item_id: string }>(
      `SELECT item_id FROM hololive_music_videos
       WHERE COALESCE(source_kind, 'official') != 'user'
       UNION
       SELECT r.item_id
       FROM source_refs r
       LEFT JOIN hololive_music_videos v ON v.item_id = r.item_id
       WHERE r.source_id = 'holodex'
         AND COALESCE(v.source_kind, 'official') != 'user'`
    );
    ...
    this.run(
      `DELETE FROM source_refs
       WHERE source_id = 'holodex'
         AND item_id NOT IN (
           SELECT item_id FROM hololive_music_videos WHERE COALESCE(source_kind, 'official') = 'user'
         )`
    );
    this.run("DELETE FROM hololive_music_videos WHERE COALESCE(source_kind, 'official') != 'user'");
    this.run(
      `DELETE FROM hololive_music_detail_cache
       WHERE youtube_video_id NOT IN (
         SELECT youtube_video_id FROM hololive_music_videos WHERE COALESCE(source_kind, 'official') = 'user'
       )`
    );
    this.run(
      `DELETE FROM hololive_music_duplicate_removals
       WHERE removed_youtube_video_id NOT IN (
         SELECT youtube_video_id FROM hololive_music_videos WHERE COALESCE(source_kind, 'official') = 'user'
       )
         AND COALESCE(kept_youtube_video_id, '') NOT IN (
           SELECT youtube_video_id FROM hololive_music_videos WHERE COALESCE(source_kind, 'official') = 'user'
         )`
    );
  ...
  listHololiveMusicRows(filters: {
    ...
    where.push("COALESCE(v.official_catalog_active, 1) = 1");

    const filterIdolIds = getHololiveExpandedTalentIds(filters.idolId);
    if (filterIdolIds.length > 0) {
      const idolPlaceholders = filterIdolIds.map(() => "?").join(", ");
      if (filters.assignment === "owned") {
        where.push(`EXISTS (SELECT 1 FROM json_each(v.owned_idol_ids_json) owned WHERE owned.value IN (${idolPlaceholders}))`);
        params.push(...filterIdolIds);
      } else if (filters.assignment === "featured") {
        where.push(`EXISTS (SELECT 1 FROM json_each(v.featured_idol_ids_json) featured WHERE featured.value IN (${idolPlaceholders}))`);
        params.push(...filterIdolIds);
      } else {
        ...
        if (participantRoles.length > 0) {
          where.push(
            `EXISTS (
              ...
              WHERE json_extract(p.value, '$.idolId') IN (${idolPlaceholders})
                AND json_extract(p.value, '$.role') IN (${participantRoles.map(() => "?").join(", ")})
            )`
          );
          params.push(...filterIdolIds, ...participantRoles);
        } else {
          where.push(
            `(v.idol_id IN (${idolPlaceholders})
              OR EXISTS (SELECT 1 FROM json_each(v.participant_idol_ids_json) participant WHERE participant.value IN (${idolPlaceholders}))
              OR EXISTS (SELECT 1 FROM json_each(v.owned_idol_ids_json) owned WHERE owned.value IN (${idolPlaceholders}))
              OR EXISTS (SELECT 1 FROM json_each(v.featured_idol_ids_json) featured WHERE featured.value IN (${idolPlaceholders})))`
          );
          params.push(...filterIdolIds, ...filterIdolIds, ...filterIdolIds, ...filterIdolIds);
        }
      }
    }
    ...
    const selectedRows = this.select<{
      ...
      source_kind: HololiveMusicSourceKind | null;
      updated_at: string;
    }>(
      `SELECT v.youtube_video_id, v.idol_id, v.title, v.song_name, v.canonical_song_key, v.canonical_performance_key,
              ...
              COALESCE(v.source_kind, 'official') AS source_kind,
              v.updated_at
    ...
    return rows.map((row) => ({
      ...
      marker: row.marker,
      sourceKind: row.source_kind === "user" ? "user" : "official",
      updatedAt: row.updated_at
    }));
  }
  ...
  private getHololiveMusicItemIdsForVideo(youtubeVideoId: string): string[] {
    ...
    return [
      ...
      this.getCustomYoutubeMusicItemId(id),
      ...this
  ...
  upsertHololiveCustomSong(input: HololiveCustomSongUpsertRequest): HololiveMusicRow {
    ...
    if (!this.isHololiveMusicTopic(input.topicId)) {
      throw new Error(`Unsupported song type: ${input.topicId}`);
    }

    const idolMetadata = this.getHololiveIdolMetadataMap();
    const ownerIdolIds = this.normalizeIdolIdArray(input.ownerIdolIds, idolMetadata);
    if (ownerIdolIds.length === 0) {
      throw new Error("Choose at least one owner talent.");
    }
    for (const idolId of ownerIdolIds) {
      this.assertHololiveIdol(idolId);
    }
    const ownerSet = new Set(ownerIdolIds);
    const featuredIdolIds = this.normalizeIdolIdArray(input.featuredIdolIds ?? [], idolMetadata).filter(
      (idolId) => !ownerSet.has(idolId)
    );
    for (const idolId of featuredIdolIds) {
      this.assertHololiveIdol(idolId);
    }

    const existing = this.select<{
      item_id: string;
      source_kind: HololiveMusicSourceKind;
    }>("SELECT item_id, COALESCE(source_kind, 'official') AS source_kind FROM hololive_music_videos WHERE youtube_video_id = ?", [
      youtubeVideoId
    ])[0];
    if (existing && existing.source_kind !== "user") {
      throw new Error("That YouTube video is already in the official Hololive music library.");
    }

    const now = new Date().toISOString();
    const itemId = existing?.item_id || this.getCustomYoutubeMusicItemId(youtubeVideoId);
    const youtubeUrl = `https://www.youtube.com/watch?v=${youtubeVideoId}`;
    const songName = input.songName?.trim() || title;
    const channelMap = this.getHololiveIdolChannelMap();
    const primaryOwnerId = ownerIdolIds[0];
    const primaryOwnerName = idolMetadata.get(primaryOwnerId)?.name ?? primaryOwnerId;
    const channelId = input.channelId?.trim() || channelMap.get(primaryOwnerId)?.channelId || `custom:${primaryOwnerId}`;
    const channelName = input.channelName?.trim() || channelMap.get(primaryOwnerId)?.channelName || primaryOwnerName;
    const participants = this.normalizeStoredHololiveMusicParticipants(
      [
        ...ownerIdolIds.map((idolId) => ({
          idolId,
          role: "primary" as const,
          channelId: channelMap.get(idolId)?.channelId ?? null
        })),
        ...featuredIdolIds.map((idolId) => ({
          idolId,
    ...
    const participantIdolIdsJson = this.stringifyIdolIdArray(this.getParticipantIdolIds(participants, idolMetadata));
    const canonicalSongKey = songName
      ? buildHololiveMusicSongKey({ songName, title })
      : buildDuplicateTitleCore(title, { channelName });
    const canonicalPerformanceKey = buildHololiveMusicPerformanceKey({
      youtubeVideoId,
      topicId: input.topicId,
      canonicalSongKey,
      ownedIdolIds: ownerIdolIds,
      effectiveOwnerChannelId: channelId,
      channelId
    });
    const publishedAt = this.normalizeHololiveCustomSongPublishedAt(input.publishedAt);
    const durationSeconds =
      input.durationSeconds === null || input.durationSeconds === undefined
        ? null
        : Math.max(0, Math.trunc(Number(input.durationSeconds)));
    const viewCount =
      input.viewCount === null || input.viewCount === undefined ? null : Math.max(0, Math.trunc(Number(input.viewCount)));
    const fetchedAt = input.fetchedAt?.trim() || now;
    const tags = normalizeTags([
      "hololive",
      "music",
      "custom song",
      input.topicId === "Original_Song" ? "original song" : "cover",
      title,
      songName,
      channelName,
      ...ownerIdolIds,
    ...
    this.transaction(() => {
      this.run(
        `INSERT INTO catalog_items (id, module_id, kind, title, subtitle, source_url, created_at, updated_at)
         ...
         ON CONFLICT(id) DO UPDATE SET
           ...
           updated_at = excluded.updated_at`,
        [itemId, songName || title, channelName || null, youtubeUrl, now, now]
      );
      this.run(
      ...
      this.run(
        `INSERT INTO source_refs (id, item_id, source_id, source_key, detail_url, cover_url, created_at, updated_at)
         ...
         ON CONFLICT(source_id, source_key) DO UPDATE SET
           ...
           updated_at = excluded.updated_at`,
        [randomUUID(), itemId, youtubeVideoId, youtubeUrl, now, now]
      );
      this.run("DELETE FROM item_tags WHERE item_id = ?", [itemId]);
      for (const tag of tags) {
        this.run("INSERT OR IGNORE INTO item_tags (item_id, tag) VALUES (?, ?)", [itemId, tag]);
      }
      this.run(
        `INSERT INTO hololive_music_videos
           (youtube_video_id, item_id, idol_id, youtube_url, title, status, topic_id, channel_id, channel_name,
            ...
            owned_idol_ids_json, featured_idol_ids_json, source_kind, official_catalog_active, source_run_id, updated_at)
         VALUES (?, ?, ?, ?, ?, 'past', ?, ?, ?, ?, ?, ?, NULL, 0, ?, ?, ?, ?, ?, ?, 'user', 1, NULL, ?)
         ON CONFLICT(youtube_video_id) DO UPDATE SET
           item_id = excluded.item_id,
           ...
           source_kind = 'user',
           official_catalog_active = 1,
           updated_at = excluded.updated_at
         WHERE hololive_music_videos.source_kind = 'user'`,
        [
          youtubeVideoId,
          itemId,
          primaryOwnerId,
          ...
          channelName,
          publishedAt,
          Number.isFinite(durationSeconds ?? Number.NaN) ? durationSeconds : null,
          songName,
          participantsJson,
          participantIdolIdsJson,
          canonicalSongKey,
          canonicalPerformanceKey,
          JSON.stringify(ownerIdolIds),
          JSON.stringify(featuredIdolIds),
          now
        ]
      );
      if (viewCount !== null && Number.isFinite(viewCount)) {
        this.run(
          ...
          [youtubeVideoId, viewCount, fetchedAt]
        );
      }
    });
    ...
    if (!row) {
      throw new Error("Custom song was not saved correctly.");
    }
    return row;
  }

  deleteHololiveCustomSong(youtubeVideoId: string): HololiveMusicPlayerData {
    const id = youtubeVideoId.trim();
    if (!id) {
      throw new Error("A YouTube video ID is required.");
    }

    const row = this.select<{ title: string; source_kind: HololiveMusicSourceKind }>(
      "SELECT title, COALESCE(source_kind, 'official') AS source_kind FROM hololive_music_videos WHERE youtube_video_id = ?",
      [id]
    )[0];
    if (!row) {
      throw new Error(`Unknown custom song: ${id}`);
    }
    if (row.source_kind !== "user") {
      throw new Error("Only custom songs can be deleted.");
    }

    this.registerHololiveCustomSongDeleteUndo(id);
    this.createBackup("hololive-custom-song-delete");
    this.transaction(() => {
      this.run("DELETE FROM hololive_music_playlist_items WHERE youtube_video_id = ?", [id]);
      this.run("DELETE FROM hololive_music_queue_items WHERE youtube_video_id = ?", [id]);
      const state = this.ensureHololiveMusicPlayerState();
      if (state.currentYoutubeVideoId === id) {
        this.setHololiveMusicPlayerStateRow({
          playbackSourceType: "library",
          currentQueueItemId: null,
          currentPlaylistId: null,
      ...
      this.deleteHololiveMusicVideoData(id, { deleteMarker: true, deleteStats: true });
      this.normalizeHololiveMusicQueuePositionsWithinTransaction();
    });
  ...
  private registerHololiveCustomSongDeleteUndo(youtubeVideoId: string): void {
    const id = youtubeVideoId.trim();
    const itemIds = this.getHololiveMusicItemIdsForVideo(id);
    const itemPlaceholders = itemIds.map(() => "?").join(", ");
    const markerKeys = this.getHololiveMusicMarkerKeysForVideo(id);
    ...
    const snapshot = {
      ...
      sourceRefs:
        itemIds.length > 0
          ? this.selectTableRows(
              "source_refs",
              `WHERE item_id IN (${itemPlaceholders}) OR (source_id = 'youtube' AND source_key = ?)`,
              [...itemIds, id]
            )
          : this.selectTableRows("source_refs", "WHERE source_id = 'youtube' AND source_key = ?", [id]),
      itemTags: itemIds.length > 0 ? this.selectTableRows("item_tags", `WHERE item_id IN (${itemPlaceholders})`, itemIds) : [],
      trackedEntries: itemIds.length > 0 ? this.selectTableRows("tracked_entries", `WHERE item_id IN (${itemPlaceholders})`, itemIds) : [],
      musicVideos: this.selectTableRows("hololive_music_videos", "WHERE youtube_video_id = ?", [id]),
      detailCache: this.selectTableRows("hololive_music_detail_cache", "WHERE youtube_video_id = ?", [id]),
      stats: this.selectTableRows("hololive_music_video_stats", "WHERE youtube_video_id = ?", [id]),
      duplicateRemovals: this.selectTableRows(
      ...
      markerKeys:
        markerKeys.length > 0
          ? this.selectTableRows("hololive_music_marker_keys", `WHERE marker_key IN (${markerPlaceholders})`, markerKeys)
          : []
    };

    this.registerHololiveUndoAction("custom-song-delete", "Restore custom song", () => {
      this.insertOrReplaceRows("catalog_items", snapshot.catalogItems);
      this.insertOrReplaceRows("source_refs", snapshot.sourceRefs);
      this.insertOrReplaceRows("item_tags", snapshot.itemTags);
      this.insertOrReplaceRows("tracked_entries", snapshot.trackedEntries);
      this.insertOrReplaceRows("hololive_music_videos", snapshot.musicVideos);
      this.insertOrReplaceRows("hololive_music_detail_cache", snapshot.detailCache);
      this.insertOrReplaceRows("hololive_music_video_stats", snapshot.stats);
      this.insertOrReplaceRows("hololive_music_duplicate_removals", snapshot.duplicateRemovals);
  ...
  listHololiveMusicVideoIdsForStats(input: {
    ...
    const where: string[] = [
      "NOT EXISTS (SELECT 1 FROM hololive_music_exclusions e WHERE e.youtube_video_id = v.youtube_video_id)",
      "COALESCE(v.official_catalog_active, 1) = 1"
    ];
    const params: unknown[] = [];
    const requestedIds = [
  ...
  getHololiveMusicSourceChannelMismatchReason(input: {
    youtubeVideoId: string;
    ...
    const row = this.select<{
      ...
      `SELECT channel_id, original_channel_id, provided_to_youtube, source_kind
       FROM hololive_music_videos
       WHERE youtube_video_id = ?`,
      [youtubeVideoId]
    )[0];
    if (!row || row.source_kind === "user" || !row.provided_to_youtube) {
      return null;
    }

    const expectedChannelId = row.original_channel_id?.trim() ?? "";
    if (!expectedChannelId || expectedChannelId === youtubeChannelId) {
      return null;
    }

    const actualLabel = input.youtubeChannelName?.trim()
      ? `${input.youtubeChannelName.trim()} (${youtubeChannelId})`
      : youtubeChannelId;
    return `YouTube reports uploader ${actualLabel}, but the imported source expects channel ${expectedChannelId}.`;
  }
  ...
  listHololiveMusicLibrary(filters: {
    ...
    if (filters.sourceKind) {
      if (filters.sourceKind !== "official" && filters.sourceKind !== "user") {
        throw new Error(`Unsupported Hololive music source kind: ${filters.sourceKind}`);
      }
      where.push("COALESCE(v.source_kind, 'official') = ?");
      params.push(filters.sourceKind);
    }

    const filterTalentIds = getHololiveExpandedTalentIds(filters.talentId);
    if (filterTalentIds.length > 0) {
      const talentPlaceholders = filterTalentIds.map(() => "?").join(", ");
      where.push(
        `(v.idol_id IN (${talentPlaceholders})
          OR EXISTS (SELECT 1 FROM json_each(v.participant_idol_ids_json) participant WHERE participant.value IN (${talentPlaceholders}))
          OR EXISTS (SELECT 1 FROM json_each(v.owned_idol_ids_json) owned WHERE owned.value IN (${talentPlaceholders}))
          OR EXISTS (SELECT 1 FROM json_each(v.featured_idol_ids_json) featured WHERE featured.value IN (${talentPlaceholders})))`
      );
      params.push(...filterTalentIds, ...filterTalentIds, ...filterTalentIds, ...filterTalentIds);
    }
    ...
    if (filters.collabScope === "solo") {
      where.push(
        `(
          ...
          OR (
            json_array_length(v.owned_idol_ids_json) > 0
            AND json_array_length(v.featured_idol_ids_json) = 0
            AND NOT EXISTS (
              SELECT 1 FROM json_each(v.owned_idol_ids_json) owned
              WHERE owned.value NOT IN ('fuwawa-abyssgard', 'mococo-abyssgard')
            )
            AND NOT EXISTS (
              SELECT 1 FROM json_each(v.participant_idol_ids_json) participant
              WHERE participant.value NOT IN ('fuwawa-abyssgard', 'mococo-abyssgard')
            )
            AND COALESCE(c.kind, 'unknown') != 'group'
          )
        )`
    ...
    if (filters.query?.trim()) {
      ...
      const idolMatches = HOLOLIVE_IDOLS
        ...
        .map((idol) => idol.id);
      if ("fuwamoco fuwa moco fuwawa mococo".includes(normalizedQuery)) {
        idolMatches.push(HOLOLIVE_FUWAMOCO_TALENT_ID);
      }
      const expandedIdolMatches = [...new Set(idolMatches.flatMap((idolId) => getHololiveExpandedTalentIds(idolId)))];
      const idolSearchSql = expandedIdolMatches
        .map(
          () =>
      ...
      params.push(query, query, query, query, query, query, query, query, query);
      for (const idolId of expandedIdolMatches) {
  ...
  getHololiveMusicPlayerData(): HololiveMusicPlayerData {
    ...
    if (state.playbackSourceType === "queue" && state.currentQueueItemId) {
      const currentQueueItem = queue.find((item) => item.id === state.currentQueueItemId);
      if (!currentQueueItem?.available) {
    ...
    const favoriteRows = this.select<{
      ...
      `SELECT v.youtube_video_id,
       ...
       WHERE COALESCE(cm.marker, vm.marker) = 'favorite'
         AND COALESCE(v.official_catalog_active, 1) = 1
         AND NOT EXISTS (
           SELECT 1 FROM hololive_music_exclusions e WHERE e.youtube_video_id = v.youtube_video_id
         )
  ...
  private compareHololiveMusicDisplayRow(
    ...
    const leftViewCount = typeof left.view_count === "number" && Number.isFinite(left.view_count) ? left.view_count : -1;
    const rightViewCount = typeof right.view_count === "number" && Number.isFinite(right.view_count) ? right.view_count : -1;
    return leftViewCount - rightViewCount || (left.published_at ?? "").localeCompare(right.published_at ?? "") || right.youtube_video_id.localeCompare(left.youtube_video_id);
  }
  ...
  private scoreHololiveMusicDisplayRow(row: {
    ...
    if (hasHololiveLowPriorityVersionMarker(title) && !hasHololiveLowPriorityLiveVersionMarker(title)) {
      score -= 240;
    }
    if (
  ...
  private canReplaceHololiveLiveVariantWithFullRow(
    fullRow: {
      title: string;
      song_name?: string | null;
      view_count?: number | null;
    ...
    const fullText = [fullRow.title, fullRow.song_name ?? ""].join(" ");
    if (/(official\s*(music\s*)?(video|mv)|\bmv\b|music video|original mv)/i.test(fullText)) {
      return true;
    }

    const fullViewCount = typeof fullRow.view_count === "number" && Number.isFinite(fullRow.view_count) ? fullRow.view_count : null;
    const liveViewCount =
      typeof liveVariantRow.view_count === "number" && Number.isFinite(liveVariantRow.view_count) ? liveVariantRow.view_count : null;
    return fullViewCount !== null && liveViewCount !== null && fullViewCount >= liveViewCount;
  }
  ...
  private inferHololivePerformerIdolsFromText(
    ...
    const text = normalizeHololiveMusicText(value ?? "").trim();
    if (!text) {
      return [];
    }

    const aliasMap = this.getHololivePerformerAliasMap(idolMetadata);
    const performerSegments = [
      text,
      ...[...text.matchAll(/[\(\[\u3010\uFF08]([^\)\]\u3011\uFF09]+)[\)\]\u3011\uFF09]/gu)].map((match) => match[1])
    ];
    const inferredIds: string[] = [];
    const addAliasesFromText = (candidateText: string) => {
      const normalizedCandidateText = buildNormalizedHololiveMusicKey(candidateText);
      if (!normalizedCandidateText) {
        return;
      }
      for (const [alias, idolIds] of aliasMap) {
        if (alias.length >= 3 && this.normalizedTextContainsAlias(normalizedCandidateText, alias)) {
          inferredIds.push(...idolIds);
        }
      }
    };

    for (const segment of performerSegments) {
      ...
      if (!/[,&、+×＆/|]|(?:^|\s)(?:x|and|with|ft\.?|feat\.?|featuring)(?:\s|$)/iu.test(segment)) {
        continue;
      }
      ...
      for (const candidate of candidates) {
        ...
        if (aliasMap.has(normalizedCandidate)) {
          ...
          addAliasesFromText(candidate);
        }
      }
      addAliasesFromText(segment);
    }
  ...
  private getHololiveIdolMetadataMap(): Map<string, { name: string; sortOrder: number; source: HololiveIdol["source"] }> {
    return new Map(
      this.select<{ id: string; display_name: string; sort_order: number; source: HololiveIdol["source"] }>(
        "SELECT id, display_name, sort_order, source FROM hololive_idols"
      ).map((row) => [row.id, { name: row.display_name, sortOrder: Number(row.sort_order), source: row.source }])
    );
  }
  ...
  private getHololiveMusicSourceKindForIdols(
    idolIds: string[],
    idolMetadata = this.getHololiveIdolMetadataMap()
  ): HololiveMusicSourceKind {
    return idolIds.some((idolId) => idolMetadata.get(idolId)?.source === "custom") ? "user" : "official";
  }

  private isUserOwnedHololiveMusicVideo(youtubeVideoId: string): boolean {
    const id = youtubeVideoId.trim();
    if (!id || !this.tableExists("hololive_music_videos") || !this.tableColumnNames("hololive_music_videos").has("source_kind")) {
      return false;
    }
    const row = this.select<{ source_kind: HololiveMusicSourceKind }>(
      "SELECT source_kind FROM hololive_music_videos WHERE youtube_video_id = ?",
      [id]
    )[0];
    return row?.source_kind === "user";
  }
  ...
  listHololiveIdols(resolveCachedImageUrl: (fileName: string) => string = (fileName) => fileName): HololiveIdol[] {
    return this.select<{
      ...
      cached_card_filename: string | null;
      ...
      `SELECT i.id, i.slug, i.display_name, i.branch, i.generation, i.status, i.source, i.official_url, i.icon_url,
              i.profile_image_url, i.card_image_url, i.profile_quote, i.youtube_channel_url, i.youtube_channel_id,
              ...
              card_cache.local_filename AS cached_card_filename,
              i.sort_order
       FROM hololive_idols i
       ...
       LEFT JOIN hololive_image_cache card_cache
         ON card_cache.idol_id = i.id
        AND card_cache.kind = 'card'
        AND card_cache.local_filename = i.slug || ?
       ORDER BY i.sort_order ASC, i.display_name ASC`,
      [CURRENT_HOLOLIVE_ICON_CACHE_SUFFIX, CURRENT_HOLOLIVE_PROFILE_CACHE_SUFFIX, CURRENT_HOLOLIVE_CARD_CACHE_SUFFIX]
    ).map((row) => ({
      id: row.id,
      slug: row.slug,
      ...
      cachedCardImageUrl: row.cached_card_filename ? resolveCachedImageUrl(row.cached_card_filename) : null,
  ...
  async duplicateHololiveBracket(bracketId: string): Promise<HololiveBracket> {
    ...
    const source = this.getHololiveBracket(id);
    const now = new Date().toISOString();
    ...
    const sourceMatches = this.select<{
      ...
      `SELECT round_index, match_index, entry_a_id, entry_b_id
       FROM hololive_bracket_matches
       WHERE bracket_id = ?
       ORDER BY round_index ASC, match_index ASC`,
      [id]
    ...
    return this.getHololiveBracket(duplicateId);
  }
  ...
  listHololiveBracketArchives(): HololiveBracketArchiveSummary[] {
    return this.select<{
      ...
      const championIdentity = row.champion_idol_id
        ? getHololiveCanonicalTalentIdentity(row.champion_idol_id, row.champion_idol_name)
        : null;
      return {
        id: row.id,
        ...
        championTitle: row.champion_title,
        championIdolId: championIdentity?.id ?? row.champion_idol_id,
        championIdolName: championIdentity?.name ?? row.champion_idol_name,
        createdAt: row.created_at,
        completedAt: row.completed_at,
  ...
  getHololiveBracketStatsOverview(format: HololiveBracketStatsFormat = "all"): HololiveBracketStatsOverview {
    ...
    const currentSongRows = this.select<CurrentBracketSongRow>(
      `SELECT v.youtube_video_id, v.title, v.song_name, v.topic_id, v.idol_id,
              v.canonical_performance_key, v.provided_to_youtube, v.duration_seconds,
              stats.view_count, v.published_at
       FROM hololive_music_videos v
       LEFT JOIN hololive_music_video_stats stats
         ON stats.youtube_video_id = v.youtube_video_id`
    );
    const currentSongsByVideoId = new Map(currentSongRows.map((row) => [row.youtube_video_id, row]));
    const duplicateTargets = new Map(
      this.select<{ removed_youtube_video_id: string; kept_youtube_video_id: string | null }>(
        `SELECT removed_youtube_video_id, kept_youtube_video_id
         FROM hololive_music_duplicate_removals
         WHERE COALESCE(kept_youtube_video_id, '') != ''`
      ).map((row) => [row.removed_youtube_video_id, row.kept_youtube_video_id ?? ""])
    );
    const resolveCurrentVideoId = (youtubeVideoId: string): string => {
      let current = youtubeVideoId;
      const visited = new Set<string>();
      while (duplicateTargets.has(current) && !visited.has(current)) {
        visited.add(current);
        current = duplicateTargets.get(current)?.trim() || current;
      }
      return current;
    };
    const representativeSongsByCanonicalKey = new Map<string, CurrentBracketSongRow>();
    for (const row of currentSongRows) {
      const key = row.canonical_performance_key.trim() || `video:${row.youtube_video_id}`;
      const existing = representativeSongsByCanonicalKey.get(key);
      if (!existing || this.compareHololiveMusicDisplayRow(row, existing) > 0) {
        representativeSongsByCanonicalKey.set(key, row);
      }
    }
    const resolveSongIdentity = (
      youtubeVideoId: string,
      archivedCanonicalPerformanceKey: string | null | undefined
    ) => {
      const resolvedVideoId = resolveCurrentVideoId(youtubeVideoId);
      const current = currentSongsByVideoId.get(resolvedVideoId);
      const canonicalPerformanceKey =
        current?.canonical_performance_key.trim() || archivedCanonicalPerformanceKey?.trim() || `video:${resolvedVideoId}`;
      const representative = representativeSongsByCanonicalKey.get(canonicalPerformanceKey) ?? current ?? null;
      return {
        canonicalPerformanceKey,
        youtubeVideoId: representative?.youtube_video_id ?? resolvedVideoId,
        representative
      };
    };
    const rows = this.select<{
      ...
      wins: number;
      losses: number;
      eliminated_round_index: number | null;
      ...
      `SELECT e.archive_id, e.youtube_video_id, e.title_snapshot, e.topic_id, e.idol_id_snapshot, e.idol_name_snapshot,
              e.canonical_performance_key,
              e.wins, e.losses, e.eliminated_round_index, e.first_round_eliminated,
    ...
    const songArchiveAppearances = new Set<string>();
    const songArchiveChampions = new Set<string>();
    const songArchiveFinalists = new Set<string>();
    const songArchiveRunnerUps = new Set<string>();
    const songArchiveTop4 = new Set<string>();
    const songArchiveTop8 = new Set<string>();
    const songArchiveTop16 = new Set<string>();
    const songArchiveEarlyExits = new Set<string>();
    ...
    for (const row of rows) {
      ...
      const talentIdentity = getHololiveCanonicalTalentIdentity(row.idol_id_snapshot, row.idol_name_snapshot);

      const songIdentity = resolveSongIdentity(row.youtube_video_id, row.canonical_performance_key);
      ...
      const song =
        ...
        ({
          canonicalPerformanceKey: songIdentity.canonicalPerformanceKey,
          youtubeVideoId: songIdentity.youtubeVideoId,
          title: songIdentity.representative?.song_name || songIdentity.representative?.title || row.title_snapshot,
          topicId: songIdentity.representative?.topic_id ?? (this.isHololiveMusicTopic(row.topic_id) ? row.topic_id : null),
          idolId: representativeTalent.id,
          idolName: representativeTalent.name,
          appearances: 0,
          wins: 0,
          losses: 0,
          winRate: 0,
      ...
      if (finalistCount > 0 && !songArchiveFinalists.has(songArchiveKey)) {
        song.finalistCount += 1;
        songArchiveFinalists.add(songArchiveKey);
      }
      if (runnerUpCount > 0 && !songArchiveRunnerUps.has(songArchiveKey)) {
        song.runnerUpCount += 1;
        songArchiveRunnerUps.add(songArchiveKey);
      }
      if (top4Count > 0 && !songArchiveTop4.has(songArchiveKey)) {
        song.top4Count += 1;
        songArchiveTop4.add(songArchiveKey);
      }
      if (top8Count > 0 && !songArchiveTop8.has(songArchiveKey)) {
        song.top8Count += 1;
        songArchiveTop8.add(songArchiveKey);
      }
      if (top16Count > 0 && !songArchiveTop16.has(songArchiveKey)) {
        song.top16Count += 1;
        songArchiveTop16.add(songArchiveKey);
      }
      if (firstRoundEliminated > 0 && !songArchiveEarlyExits.has(songArchiveKey)) {
      ...
      const talent =
        talentStats.get(talentIdentity.id) ??
        ({
          idolId: talentIdentity.id,
      ...
      const deepRunSamples = deepRunSamplesByTalent.get(talentIdentity.id) ?? [];
      deepRunSamples.push(top4Count > 0);
      deepRunSamplesByTalent.set(talentIdentity.id, deepRunSamples);
      const earlyExitSamples = earlyExitSamplesByTalent.get(talentIdentity.id) ?? [];
      earlyExitSamples.push(earlyExitCount > 0);
      earlyExitSamplesByTalent.set(talentIdentity.id, earlyExitSamples);
      if (finalistCount > 0) {
        const finalsSamples = finalsConversionSamplesByTalent.get(talentIdentity.id) ?? [];
        finalsSamples.push(championCount > 0);
        finalsConversionSamplesByTalent.set(talentIdentity.id, finalsSamples);
      }
    }
    ...
    const ensureRatingRow = (
      map: Map<string, HololiveBracketRatingStatsRow>,
      id: string,
      label: string,
      detail: string | null,
      archivedAt: string
    ...
    for (const row of matchRows) {
      ...
      const loserViews =
        loserSongIdentity.representative?.view_count ??
        (row.loser_view_count == null ? null : Number(row.loser_view_count));
      ...
      if (isDistinctSongResult && loserViews != null && Number.isFinite(loserViews) && loserViews > 0) {
        ...
        strengthOfWinsSamplesBySong.set(winnerSongKey, samples);
      }
      if (isDistinctSongResult && winnerViews != null && Number.isFinite(winnerViews) && winnerViews > 0) {
        const samples = strengthOfLossesSamplesBySong.get(loserSongKey) ?? [];
        samples.push(winnerViews);
        strengthOfLossesSamplesBySong.set(loserSongKey, samples);
      }
      if (
        winnerTalentIdentity.id &&
        isDistinctSongResult &&
        loserViews != null &&
        Number.isFinite(loserViews) &&
        loserViews > 0
      ) {
        const samples = strengthOfWinsSamplesByTalent.get(winnerTalentIdentity.id) ?? [];
        samples.push(loserViews);
        strengthOfWinsSamplesByTalent.set(winnerTalentIdentity.id, samples);
      }
      if (
        ...
        const samples = strengthOfLossesSamplesByTalent.get(loserTalentIdentity.id) ?? [];
        samples.push(winnerViews);
        strengthOfLossesSamplesByTalent.set(loserTalentIdentity.id, samples);
      }
      if (
        isDistinctSongResult &&
        winnerViews != null &&
        ...
        const underdogTalentId = winnerViews < loserViews ? winnerTalentIdentity.id : loserTalentIdentity.id;
        if (underdogTalentId) {
      ...
      if (
        ...
        if (winnerTalentIdentity.id) {
          ...
          clutchSamplesByTalent.set(winnerTalentIdentity.id, samples);
        }
        if (loserTalentIdentity.id) {
          const samples = clutchSamplesByTalent.get(loserTalentIdentity.id) ?? [];
          samples.push({ value: -1, weight: roundWeight });
      ...
      if (
        ...
        const favoredTalentId = winnerViews > loserViews ? winnerTalentIdentity.id : loserTalentIdentity.id;
        if (favoredTalentId) {
      ...
      const winnerIdolId = winnerTalentIdentity.id;
      const loserIdolId = loserTalentIdentity.id;
      ...
      const addRivalryResult = (target: Map<string, HololiveBracketRivalryStats>) => {
        const targetRivalry =
          target.get(key) ??
          ({
            key,
    ...
    const finalsWithoutTitle = (song: HololiveBracketSongStats) => song.runnerUpCount;
    ...
    const pushSongPressureEdgeSample = (songId: string, value: number, weight: number) => {
      const samples = pressureEdgeSamplesBySong.get(songId) ?? [];
      samples.push({ value, weight });
      pressureEdgeSamplesBySong.set(songId, samples);
    };
    const pushPressureEdgeSample = (talentId: string, value: number, weight: number) => {
      const samples = pressureEdgeSamplesByTalent.get(talentId) ?? [];
      samples.push({ value, weight });
    ...
    for (const talent of talents) {
      const updated = talentStats.get(talent.idolId);
      if (updated) {
        talent.pressureEdgeScore = updated.pressureEdgeScore;
        talent.pressureEdgeMatches = updated.pressureEdgeMatches;
        talent.pressureEdgePositiveMatches = updated.pressureEdgePositiveMatches;
        talent.pressureEdgeNegativeMatches = updated.pressureEdgeNegativeMatches;
      }
    }
    for (const song of songs) {
      const updated = songStats.get(song.canonicalPerformanceKey);
      if (updated) {
        song.pressureEdgeScore = updated.pressureEdgeScore;
        song.pressureEdgeMatches = updated.pressureEdgeMatches;
        song.pressureEdgePositiveMatches = updated.pressureEdgePositiveMatches;
        song.pressureEdgeNegativeMatches = updated.pressureEdgeNegativeMatches;
      }
    }
    ...
    const buildRatingRows = (ratingRows: Map<string, HololiveBracketRatingStatsRow>, ratings: Map<string, Glicko2Rating>) =>
      [...ratingRows.values()]
        .flatMap((row) => {
          ...
          return [
            {
              ...
              conservativeRating: getConservativeGlicko2Rating(rating),
    ...
    return {
      ...
      topSongRatings: buildRatingRows(songRatingRows, songRatings),
  ...
  private generateHololiveBracketEntries(
  ...
  ): HololiveBracketGenerationResult {
    const sizeCount = HOLOLIVE_BRACKET_SIZE_COUNTS[size];
    const pools = this.loadHololiveBracketCandidatePools(generationFilters);
    const selection = this.selectHololiveBracketCandidates(sizeCount, seed, generationStyle, generationFilters, pools);
    ...
    return {
      entries: this.seedHololiveBracketFirstRoundSlots(selected, seed, size, generationStyle).map((candidate, slotIndex) =>
  ...
  private selectHololiveBracketCandidates(
    sizeCount: number,
    seed: string,
    generationStyle: HololiveBracketGenerationStyle,
    generationFilters: HololiveBracketGenerationFilters,
    pools: HololiveBracketCandidatePools
  ): HololiveBracketCandidateSelectionResult {
    const options = this.getHololiveBracketSelectionOptions(generationFilters, sizeCount);
    const generate = (selectionOptions: HololiveBracketSelectionOptions) =>
      generationStyle === "random_songs"
        ? this.generateRandomHololiveBracketCandidates(sizeCount, seed, pools, selectionOptions)
        : this.generateTopHololiveBracketCandidates(sizeCount, seed, pools, selectionOptions);
    const selected = generate(options);
    if (selected.length >= sizeCount || options.maxEntriesPerTalent === null) {
  ...
  private getHololiveBracketSelectionOptions(
    generationFilters: HololiveBracketGenerationFilters,
    sizeCount: number
  ): HololiveBracketSelectionOptions {
    const rawMax = generationFilters.maxEntriesPerTalent;
    const maxEntriesPerTalent =
      rawMax === undefined
        ? Math.min(2, sizeCount)
        : typeof rawMax === "number" && Number.isInteger(rawMax) && rawMax > 0
          ? Math.min(rawMax, sizeCount)
          : null;
    return {
      maxEntriesPerTalent,
      preferTopicSplitPerTalent: generationFilters.preferTopicSplitPerTalent !== false
    };
  }
  ...
  private loadHololiveBracketCandidatePools(generationFilters: HololiveBracketGenerationFilters): HololiveBracketCandidatePools {
    const includedTalentIds = Array.isArray(generationFilters.includedTalentIds)
      ? new Set(generationFilters.includedTalentIds.map((idolId) => getHololiveCanonicalTalentId(idolId)).filter(Boolean))
      : null;
    const officialIdols = this.getHololiveBracketIdolsBySubscriberPriority().filter(
      (idol) => {
        const bracketTalentId = getHololiveCanonicalTalentId(idol.id);
        return (
          (includedTalentIds === null || includedTalentIds.has(bracketTalentId)) &&
          this.matchesHololiveBracketTalentStatusFilter(idol, generationFilters)
        );
      }
    );
  ...
  private matchesHololiveBracketTalentStatusFilter(
    idol: HololiveIdol,
    filters: HololiveBracketGenerationFilters
  ): boolean {
    if (!filters.talentStatuses?.length || filters.talentStatuses.length >= HOLOLIVE_BRACKET_TALENT_STATUS_FILTERS.length) {
      return true;
    }
    return new Set(filters.talentStatuses).has(this.getHololiveBracketTalentStatusFilter(idol));
  }

  private getHololiveBracketTalentStatusFilter(idol: HololiveIdol): HololiveBracketTalentStatusFilter {
    if (idol.source === "custom") {
      return "custom";
    }
    return idol.status === "alum" || idol.status === "retired" ? "alumni" : "active";
  }
  ...
  private getHololiveBracketHistoryExclusions(
    ...
    const exclusions: HololiveBracketHistoryExclusions = {
      ...
      participation: this.getHololiveBracketHistoryParticipation(filters.historyParticipation)
    };
    if (clauses.length === 0) {
      return exclusions;
    }
  ...
  private getHololiveBracketHistoryParticipation(
    mode: HololiveBracketHistoryParticipation | null | undefined
  ): HololiveBracketHistoryExclusions["participation"] {
    if (!mode) {
      return null;
    }

    const clauses: Record<HololiveBracketHistoryParticipation, string> = {
      never: "1 = 1",
      appeared: "1 = 1",
      top8: "is_top8 = 1",
      top4: "is_top4 = 1",
      finalist: "is_finalist = 1",
      winner: "is_champion = 1"
    };
    const rows = this.select<{ youtube_video_id: string; canonical_performance_key: string | null }>(
      `SELECT youtube_video_id, canonical_performance_key
       FROM hololive_bracket_archive_entries
    ...
    const canonicalPerformanceKeys = new Set<string>();
    for (const row of rows) {
      youtubeVideoIds.add(row.youtube_video_id);
      const key = row.canonical_performance_key?.trim();
      if (key) {
        canonicalPerformanceKeys.add(key);
      }
    }
  ...
  private generateTopHololiveBracketCandidates(
    ...
    seed: string,
    pools: HololiveBracketCandidatePools,
    options: HololiveBracketSelectionOptions
  ): HololiveBracketCandidate[] {
    return this.selectHololiveBracketCandidatesFromOrderedList(
      sizeCount,
      this.sortHololiveBracketCandidates(pools.officialIdols.flatMap((idol) => pools.allByIdol.get(idol.id) ?? [])),
      options
    );
  }

  private selectHololiveBracketCandidatesFromOrderedList(
    sizeCount: number,
    orderedCandidates: HololiveBracketCandidate[],
    options: HololiveBracketSelectionOptions
  ): HololiveBracketCandidate[] {
    ...
    const canAddCandidate = (candidate: HololiveBracketCandidate | null | undefined): candidate is HololiveBracketCandidate => {
      ...
      return (
        options.maxEntriesPerTalent === null ||
        (selectedCountsByIdol.get(candidate.idolIdForBracket) ?? 0) < options.maxEntriesPerTalent
      );
    };

    const shouldDeferForTopicSplit = (candidate: HololiveBracketCandidate): boolean => {
      if (!options.preferTopicSplitPerTalent) {
        return false;
      }

      const selectedTopics = selectedTopicsByIdol.get(candidate.idolIdForBracket);
      if (!selectedTopics?.has(candidate.topicId)) {
        return false;
      }

      const oppositeTopic: HololiveMusicTopic = candidate.topicId === "Original_Song" ? "Music_Cover" : "Original_Song";
      if (selectedTopics.has(oppositeTopic)) {
        return false;
      }

      return orderedCandidates.some(
        (row) =>
          row.idolIdForBracket === candidate.idolIdForBracket &&
          row.topicId === oppositeTopic &&
          canAddCandidate(row)
      );
    };
    ...
    const addPass = (allowDeferredSameTopic: boolean): number => {
      let added = 0;
      for (const candidate of orderedCandidates) {
        if (selected.length >= sizeCount) {
          break;
        }
        if (!allowDeferredSameTopic && shouldDeferForTopicSplit(candidate)) {
          continue;
        }
  ...
  private generateRandomHololiveBracketCandidates(
    ...
    return this.selectHololiveBracketCandidatesFromOrderedList(
      sizeCount,
      this.shuffleWithHololiveBracketSeed(
        pools.officialIdols.flatMap((idol) => pools.allByIdol.get(idol.id) ?? []),
        `${seed}:random:candidates`
      ),
      options
    );
  }

  private getHololiveBracketIdolsBySubscriberPriority(): HololiveIdol[] {
  ...
  private getHololiveBracketCandidatesForIdol(
    ...
    const bracketIdentity = getHololiveCanonicalTalentIdentity(idol.id, idol.displayName);
    const rows = this.listHololiveMusicRows({
      ...
      .map<HololiveBracketCandidate>((row) => ({
        ...row,
        idolIdForBracket: bracketIdentity.id,
        idolNameForBracket: bracketIdentity.name
      }));
  ...
  private matchesHololiveBracketGenerationFilters(
    ...
    if (filters.vocalScopes?.length) {
      const allowedVocalScopes = new Set(filters.vocalScopes);
      const rowScope = this.getHololiveBracketVocalScope(row);
      if (!allowedVocalScopes.has(rowScope)) {
        return false;
      }
    }
    ...
    if (historyExclusions.participation) {
      const appeared =
        historyExclusions.participation.youtubeVideoIds.has(row.youtubeVideoId) ||
        historyExclusions.participation.canonicalPerformanceKeys.has(canonicalKey);
      if (historyExclusions.participation.mode === "never" ? appeared : !appeared) {
        return false;
      }
    }
  ...
  private getHololiveBracketVocalScope(row: HololiveMusicRow): HololiveBracketVocalScope {
    const participants = new Set<string>();
    for (const participant of row.participants ?? []) {
      const idolId = getHololiveCanonicalTalentId(participant.idolId);
      if (idolId) {
        participants.add(idolId);
      }
    }
    for (const idolId of row.ownedIdolIds ?? []) {
      const canonicalId = getHololiveCanonicalTalentId(idolId);
      if (canonicalId) {
        participants.add(canonicalId);
      }
    }
    for (const idolId of row.featuredIdolIds ?? []) {
      const canonicalId = getHololiveCanonicalTalentId(idolId);
      if (canonicalId) {
        participants.add(canonicalId);
      }
    }
    return participants.size > 1 || this.hasHololiveBracketGroupVocalSignal(row) ? "group" : "solo";
  }

  private hasHololiveBracketGroupVocalSignal(row: HololiveMusicRow): boolean {
    const text = [row.songName, row.title, row.canonicalSongKey]
    ...
    return (
      /\b(?:feat|ft|featuring)\.?\b/iu.test(text) ||
      /\bduet\s+with\b/iu.test(text) ||
      /\s+x\s+/iu.test(text) ||
      /\s+[&\uFF06\u00D7]\s+/u.test(text) ||
      /[^\s][&\uFF06\u00D7][^\s]/u.test(text)
    );
  ...
  private listHololiveBracketEntries(bracketId: string): HololiveBracketEntry[] {
    return this.select<{
      ...
      const talentIdentity = getHololiveCanonicalTalentIdentity(row.idol_id, row.idol_name);
      return {
        id: row.id,
        bracketId: row.bracket_id,
  ...
  private normalizeHololiveBracketGenerationFilters(input: unknown): HololiveBracketGenerationFilters {
    ...
    const vocalScopes = this.normalizeHololiveBracketVocalScopes(record.vocalScopes);
    const hasVocalScopes = vocalScopes.length > 0;
    const talentStatuses = this.normalizeHololiveBracketTalentStatuses(record.talentStatuses);
    const hasTalentStatuses = talentStatuses.length > 0;
    const hasMaxEntriesPerTalent = Object.prototype.hasOwnProperty.call(record, "maxEntriesPerTalent");
    ...
    return {
      ...
      vocalScopes: hasVocalScopes ? vocalScopes : undefined,
      talentStatuses: hasTalentStatuses ? talentStatuses : undefined,
      historyParticipation: this.normalizeHololiveBracketHistoryParticipation(record.historyParticipation),
      maxEntriesPerTalent: hasMaxEntriesPerTalent
        ? this.normalizeHololiveBracketMaxEntriesPerTalent(record.maxEntriesPerTalent)
        : undefined,
      preferTopicSplitPerTalent: typeof record.preferTopicSplitPerTalent === "boolean" ? record.preferTopicSplitPerTalent : undefined,
  ...
  private normalizeHololiveBracketVocalScopes(value: unknown): HololiveBracketVocalScope[] {
    if (!Array.isArray(value)) {
      return [];
    }
    const selected = new Set(value.filter((scope): scope is HololiveBracketVocalScope => this.isHololiveBracketVocalScope(scope)));
    return HOLOLIVE_BRACKET_VOCAL_SCOPES.filter((scope) => selected.has(scope));
  }

  private normalizeHololiveBracketTalentStatuses(value: unknown): HololiveBracketTalentStatusFilter[] {
    if (!Array.isArray(value)) {
      return [];
    }
    const selected = new Set(
      value.filter((status): status is HololiveBracketTalentStatusFilter => this.isHololiveBracketTalentStatusFilter(status))
    );
    return HOLOLIVE_BRACKET_TALENT_STATUS_FILTERS.filter((status) => selected.has(status));
  }

  private normalizeHololiveBracketHistoryParticipation(value: unknown): HololiveBracketHistoryParticipation | null {
    return this.isHololiveBracketHistoryParticipation(value) ? value : null;
  }

  private normalizeHololiveBracketMaxEntriesPerTalent(value: unknown): number | null {
    const parsed = typeof value === "number" ? value : typeof value === "string" ? Number(value) : Number.NaN;
    return Number.isFinite(parsed) && parsed >= 1 ? Math.min(256, Math.floor(parsed)) : null;
  }
  ...
  private stringifyHololiveBracketGenerationFilters(filters: HololiveBracketGenerationFilters): string {
    ...
    if ((normalized.vocalScopes?.length ?? 0) > 0 && (normalized.vocalScopes?.length ?? 0) < HOLOLIVE_BRACKET_VOCAL_SCOPES.length) {
      payload.vocalScopes = normalized.vocalScopes;
    }
    if (
      (normalized.talentStatuses?.length ?? 0) > 0 &&
      (normalized.talentStatuses?.length ?? 0) < HOLOLIVE_BRACKET_TALENT_STATUS_FILTERS.length
    ) {
      payload.talentStatuses = normalized.talentStatuses;
    }
    if (normalized.historyParticipation) {
      payload.historyParticipation = normalized.historyParticipation;
    }
    if (normalized.maxEntriesPerTalent !== undefined) {
      payload.maxEntriesPerTalent = normalized.maxEntriesPerTalent;
    }
    if (normalized.preferTopicSplitPerTalent === false) {
      payload.preferTopicSplitPerTalent = false;
    }
  ...
  private isHololiveBracketVocalScope(value: unknown): value is HololiveBracketVocalScope {
    return value === "solo" || value === "group";
  }

  private isHololiveBracketTalentStatusFilter(value: unknown): value is HololiveBracketTalentStatusFilter {
    return value === "active" || value === "alumni" || value === "custom";
  }

  private isHololiveBracketHistoryParticipation(value: unknown): value is HololiveBracketHistoryParticipation {
    return HOLOLIVE_BRACKET_HISTORY_PARTICIPATION_VALUES.includes(value as HololiveBracketHistoryParticipation);
  }
  ...
  private scoreHololiveMusicProfileRow(row: HololiveMusicRow): number {
    ...
    if (hasHololiveLowPriorityVersionMarker(searchableTitle) && !hasHololiveLowPriorityLiveVersionMarker(searchableTitle)) {
      score -= 240;
    }
    if (
      /topic|provided to youtube|shorts?|teaser|trailer|preview|promo(?:tional)?|宣伝|ending|movie edition|midnight ver|remaster(?:ed)?|instrumental|off vocal|karaoke/i.test(
  ...
  exportDatabaseBackupToFile(destinationPath: string): { exported: boolean; filePath: string } {
    if (!this.SQL) {
      throw new Error("Database engine is not ready.");
    }

    this.flush();
    return exportDatabaseBackupToFile(this.databasePath, destinationPath, this.SQL);
  }
    ...
    this.transaction(() => {
      for (const idol of HOLOLIVE_IDOLS) {
        this.run(
          `INSERT INTO hololive_idols
             (id, slug, display_name, branch, generation, status, source, official_url, icon_url,
              profile_image_url, card_image_url, profile_quote, youtube_channel_url, youtube_channel_id, x_handle, x_url, birthday,
           ...
           WHERE hololive_idols.slug IS NOT excluded.slug
              ...
              OR hololive_idols.card_image_url IS NOT excluded.card_image_url
          ...
          [
            ...
            idol.cardImageUrl ?? idol.profileImageUrl ?? null,
  ...
  private hasHololiveMusicPersonalReferences(row: {
    youtube_video_id: string;
    item_id: string;
    canonical_performance_key: string | null;
  }): boolean {
    const count = (sql: string, params: unknown[] = []) =>
      Number(this.select<{ count: number }>(sql, params)[0]?.count ?? 0);
    ...
    if (
      ...
      count(
        `SELECT COUNT(*) AS count
         FROM hololive_music_marker_keys
         WHERE marker_key IN (${markerKeys.map(() => "?").join(", ")})`,
        markerKeys
      ) > 0
    ) {
      return true;
    ...
    if (videoReferenceQueries.some(([sql, params]) => count(sql, params) > 0)) {
      return true;
    }
  ...
  private purgeRejectedHolodexMusicRows(): void {
    ...
    const rows = this.select<{
      ...
      `SELECT v.youtube_video_id,
       ...
       WHERE COALESCE(v.source_kind, 'official') != 'user'`
    );
    const channelContext = this.getHololiveChannelContext();
    const idolMetadata = this.getHololiveIdolMetadataMap();
    ...
    const rejectedIds = rejectedRows.map((row) => row.youtube_video_id);
    const hasOrphanStats =
      this.tableExists("hololive_music_video_stats") &&
      this.scalarCount(
        `SELECT COUNT(*)
         FROM hololive_music_video_stats stats
         WHERE NOT EXISTS (
           SELECT 1
           FROM hololive_music_videos videos
           WHERE videos.youtube_video_id = stats.youtube_video_id
         )`
      ) > 0;

    if (rejectedIds.length === 0 && !hasOrphanStats) {
      return;
    }
    ...
    const purgeRows = () => {
      ...
      for (const row of rejectedRows) {
        if (this.hasHololiveMusicPersonalReferences(row)) {
          this.run(
            `UPDATE hololive_music_videos
          ...
          this.deleteHololiveMusicVideoData(row.youtube_video_id, { deleteStats: true });
        }
      }
      if (hasOrphanStats) {
        this.run(
          `DELETE FROM hololive_music_video_stats
           WHERE NOT EXISTS (
             SELECT 1
             FROM hololive_music_videos
             WHERE hololive_music_videos.youtube_video_id = hololive_music_video_stats.youtube_video_id
           )`
        );
      }
  ...
  private buildHololiveMusicParticipants(
    ...
    const idolChannels = this.getHololiveIdolChannelMap();
    const referenceText = [metadata.title, metadata.songName, ...detail.songNames]
      .map((value) => value?.trim() ?? "")
      .filter(Boolean)
      .join(" ");
    const hasParticipantId = (idolId: string) =>
      [...participants.values()].some((participant) => participant.idolId === idolId);
    ...
    const addFeatured = (
      idolIds: string[] | null | undefined,
      role: Extract<HololiveMusicParticipantRole, "mentioned" | "collab">,
      channelId: string | null
    ) => {
      for (const idolId of idolIds ?? []) {
        if (hasParticipantId(idolId)) {
          continue;
        }
        const resolvedChannelId = channelId ?? idolChannels.get(idolId)?.channelId ?? null;
        participants.set(`${idolId}:${role}`, {
          idolId,
          role,
    ...
    const trustAllMentions = this.shouldTrustHolodexMentionsForParticipants(detail, channelContext, topicId);
    if (trustAllMentions) {
      for (const mention of detail.mentions) {
        addFeatured(channelContext.linkedChannelToIdols.get(mention.channelId), "mentioned", mention.channelId);
      }
    } else {
      for (const mention of detail.mentions) {
        const mentionedIdolIds = channelContext.linkedChannelToIdols.get(mention.channelId);
        if (this.holodexMentionMatchesMusicText(mention, mentionedIdolIds ?? [], referenceText, idolMetadata)) {
          addFeatured(mentionedIdolIds, "mentioned", mention.channelId);
        }
      }
    }
  ...
  private holodexMentionMatchesMusicText(
    mention: HolodexMentionedChannel,
    idolIds: string[],
    value: string | null | undefined,
    idolMetadata = this.getHololiveIdolMetadataMap()
  ): boolean {
    ...
    if (this.textReferencesAnyHololiveIdol(value, idolIds, idolMetadata)) {
      return true;
    }
    ...
    const aliasCandidates = [
      mention.englishName,
      mention.name,
      ...mention.englishName.split(/\s+/u),
      ...mention.name.split(/[\s.\/|｜・]+/u)
    ]
      .map((alias) => buildNormalizedHololiveMusicKey(alias))
      .filter((alias, index, aliases) => alias.length >= 3 && aliases.indexOf(alias) === index);

    return aliasCandidates.some((alias) => this.normalizedTextContainsAlias(normalizedText, alias));
  }
    ...
    for (const detail of Object.values(detailsById)) {
      ...
      this.run(
        `INSERT INTO hololive_music_detail_cache
         ...
         WHERE NOT EXISTS (
           SELECT 1
           FROM hololive_music_videos user_video
           WHERE COALESCE(user_video.source_kind, 'official') = 'user'
             AND user_video.youtube_video_id = excluded.youtube_video_id
         )`,
        [
  ...
  private upsertHolodexDuplicateRemovals(removals: HolodexDuplicateRemoval[], runId: string): void {
    ...
    for (const removal of removals) {
      if (
        this.isUserOwnedHololiveMusicVideo(removal.removedYoutubeVideoId) ||
        (removal.keptYoutubeVideoId ? this.isUserOwnedHololiveMusicVideo(removal.keptYoutubeVideoId) : false)
      ) {
        continue;
      }
  ...
  private applyStoredHololiveDuplicateRemovalRows(runId: string): number {
    const rows = this.select<{
      ...
      reason: string;
      song_name: string | null;
      removed_published_at: string | null;
      kept_published_at: string | null;
    }>(
      `SELECT r.removed_youtube_video_id, r.removed_title, r.kept_youtube_video_id, r.kept_title,
              r.reason, r.song_name, r.removed_published_at, r.kept_published_at
       FROM hololive_music_duplicate_removals r
       INNER JOIN hololive_music_videos removed
         ON removed.youtube_video_id = r.removed_youtube_video_id
        AND COALESCE(removed.source_kind, 'official') != 'user'
       INNER JOIN hololive_music_videos kept
         ON kept.youtube_video_id = r.kept_youtube_video_id
        AND COALESCE(kept.source_kind, 'official') != 'user'
       WHERE COALESCE(r.kept_youtube_video_id, '') != ''
         AND r.removed_youtube_video_id != r.kept_youtube_video_id
       ORDER BY r.removed_youtube_video_id`
    );
    if (rows.length === 0) {
      return 0;
    }

    const removals: HolodexDuplicateRemoval[] = rows.map((row) => ({
      removedYoutubeVideoId: row.removed_youtube_video_id,
      removedTitle: row.removed_title,
      keptYoutubeVideoId: row.kept_youtube_video_id ?? "",
  ...
  private upsertHolodexMusicRows(
    ...
    for (const row of rows) {
      ...
      if (this.isUserOwnedHololiveMusicVideo(row.youtubeVideoId)) {
        continue;
      }
      ...
      const sourceKind = this.getHololiveMusicSourceKindForIdols(
        [idolId, ...classification.ownedIdolIds, ...classification.featuredIdolIds, ...participantTags]
      ...
      this.run(
        `INSERT INTO catalog_items (id, module_id, kind, title, subtitle, source_url, created_at, updated_at)
         ...
         WHERE NOT EXISTS (
           ...
           FROM hololive_music_videos user_video
           WHERE COALESCE(user_video.source_kind, 'official') = 'user'
             AND user_video.item_id = catalog_items.id
         )`,
      ...
      this.run(
        `INSERT INTO source_refs (id, item_id, source_id, source_key, detail_url, cover_url, created_at, updated_at)
         ...
         WHERE NOT EXISTS (
           ...
           WHERE COALESCE(user_video.source_kind, 'official') = 'user'
             AND (
               user_video.item_id = source_refs.item_id
               OR user_video.youtube_video_id = excluded.source_key
             )
         )`,
  ...
  private purgeCrossOwnerTopicDuplicateRows(runId: string): number {
    const rows = this.select<HololiveStoredMusicDuplicateRow>(
      `SELECT v.youtube_video_id, v.title, v.song_name, v.topic_id, v.canonical_song_key, v.canonical_performance_key,
              v.channel_id, v.channel_name, v.original_channel_id, v.provided_to_youtube, v.duration_seconds, stats.view_count, v.published_at,
              participants_json, owned_idol_ids_json, featured_idol_ids_json
       FROM hololive_music_videos v
       LEFT JOIN hololive_music_video_stats stats
         ON stats.youtube_video_id = v.youtube_video_id
       WHERE v.canonical_song_key != ''
         AND COALESCE(v.source_kind, 'official') != 'user'
       ORDER BY v.topic_id, v.canonical_song_key, v.provided_to_youtube, v.youtube_video_id`
    );
    if (rows.length === 0) {
      return 0;
    }
  ...
  private purgeCanonicalVariantDuplicateRows(runId: string): number {
    const rows = this.select<{
      ...
      `SELECT v.youtube_video_id, v.title, v.song_name, v.topic_id, v.canonical_song_key, v.canonical_performance_key,
       ...
       WHERE v.canonical_song_key != ''
         AND COALESCE(v.source_kind, 'official') != 'user'
       ORDER BY v.canonical_song_key, v.canonical_performance_key, v.youtube_video_id`
    );
    if (rows.length === 0) {
      return 0;
    }
    ...
    const collectVariantRemovals = (componentRows: typeof rows) => {
      ...
      for (const variantRow of lowPriorityRows) {
        const eligibleFullRows = hasHololiveLowPriorityLiveVersionMarker([variantRow.title, variantRow.song_name ?? ""].join(" "))
          ? fullRows.filter((fullRow) => this.canReplaceHololiveLiveVariantWithFullRow(fullRow, variantRow))
          : fullRows;
        const keptRow = [...eligibleFullRows].sort((left, right) => this.compareHololiveMusicDisplayRow(right, left))[0];
        if (!keptRow) {
          continue;
        }
  ...
  private purgeSupersededDirectUploadRows(runId: string): number {
    const rows = this.select<{
      youtube_video_id: string;
      title: string;
      song_name: string | null;
      ...
      `SELECT v.youtube_video_id, v.title, v.song_name, v.topic_id, v.canonical_song_key,
              v.channel_id, v.owned_idol_ids_json, v.provided_to_youtube, v.duration_seconds, stats.view_count,
              v.published_at, details.song_names_json
       FROM hololive_music_videos v
       LEFT JOIN hololive_music_video_stats stats
         ON stats.youtube_video_id = v.youtube_video_id
       LEFT JOIN hololive_music_detail_cache details
         ON details.youtube_video_id = v.youtube_video_id
       WHERE v.provided_to_youtube = 0
         AND COALESCE(v.source_kind, 'official') != 'user'
       ORDER BY v.topic_id, v.channel_id, v.published_at, v.youtube_video_id`
    );
    if (rows.length < 2) {
      return 0;
    }

    const hasStructuredSongName = (row: (typeof rows)[number]) =>
      Boolean(row.song_name?.trim()) || this.parseJsonStringArray(row.song_names_json).length > 0;
    const ownerKey = (row: (typeof rows)[number]) => {
      const idolIds = this.parseJsonStringArray(row.owned_idol_ids_json).sort();
      return idolIds.length > 0 ? idolIds.join("+") : row.channel_id?.trim() ?? "";
    };
    const groupKey = (row: (typeof rows)[number], songKey: string) =>
      JSON.stringify([row.topic_id, row.channel_id?.trim() ?? "", ownerKey(row), songKey]);
    const structuredRowsByKey = new Map<string, typeof rows>();

    for (const row of rows) {
      if (!hasStructuredSongName(row)) {
        continue;
      }
      const songKey = this.buildHololiveDirectDuplicateKey(row.canonical_song_key);
      if (!songKey || !row.channel_id?.trim() || !ownerKey(row)) {
        continue;
      }
      const key = groupKey(row, songKey);
      structuredRowsByKey.set(key, [...(structuredRowsByKey.get(key) ?? []), row]);
    }

    const removals: HolodexDuplicateRemoval[] = [];
    for (const staleRow of rows) {
      ...
      const eligibleCandidates = candidates.filter((candidate) => {
        ...
        if (!Number.isFinite(stalePublishedAt) || !Number.isFinite(candidatePublishedAt) || candidatePublishedAt <= stalePublishedAt) {
          return false;
        }

        const staleDuration = staleRow.duration_seconds ?? 0;
        const candidateDuration = candidate.duration_seconds ?? 0;
        const substantiallyLonger =
          staleDuration > 0 && candidateDuration > 0 && candidateDuration >= staleDuration * 1.15;
        const staleViews = staleRow.view_count ?? 0;
        const candidateViews = candidate.view_count ?? 0;
        const materiallyMoreViewed = staleViews > 0 && candidateViews >= staleViews * 1.5;
        if (!substantiallyLonger && !materiallyMoreViewed) {
          return false;
        }
        if (staleViews > 0 && candidateViews > 0 && candidateViews <= staleViews) {
          return false;
        }

        return this.compareHololiveMusicDisplayRow(candidate, staleRow) > 0;
      });
      const keptRow = [...eligibleCandidates].sort((left, right) =>
        this.compareHololiveMusicDisplayRow(right, left)
      )[0];
      if (!keptRow) {
        continue;
      ...
      removals.push({
        removedYoutubeVideoId: staleRow.youtube_video_id,
        removedTitle: staleRow.title,
        keptYoutubeVideoId: keptRow.youtube_video_id,
        keptTitle: keptRow.title,
        reason: "superseded_direct_upload",
        songName: keptRow.song_name || keptRow.title,
        removedPublishedAt: staleRow.published_at || "",
        keptPublishedAt: keptRow.published_at || ""
      });
    }
  ...
  private buildHololiveDirectDuplicateKey(value: string | null | undefined): string {
    const key = buildNormalizedHololiveMusicKey(value ?? "");
    if (!key || /[a-z]/iu.test(key) || !/[\p{Script=Han}\p{Script=Hiragana}\p{Script=Katakana}]/u.test(key)) {
      return key;
    }
  ...
  private getCustomYoutubeMusicItemId(youtubeVideoId: string): string {
    return `youtube:${youtubeVideoId.trim()}`;
  }

  private normalizeHololiveCustomSongPublishedAt(value?: string | null): string {
    const trimmed = value?.trim() ?? "";
    if (!trimmed) {
  ...
  private getHololiveIdolChannelMap(): Map<string, { channelId: string | null; channelName: string }> {
    return new Map(
      this.select<{ id: string; display_name: string; youtube_channel_id: string | null }>(
        "SELECT id, display_name, youtube_channel_id FROM hololive_idols"
      ).map((row) => [
        row.id,
        {
          channelId: row.youtube_channel_id?.trim() || null,
  ...
  private migrateHololiveMusicVideosToMergedParticipants(): void {
    ...
    const needsRewrite =
      ...
      !existingColumns.has("source_kind") ||
      !existingColumns.has("official_catalog_active");
    ...
    const rows = this.select<{
      ...
      featured_idol_ids_json: string | null;
      source_kind: HololiveMusicSourceKind | null;
      official_catalog_active: number | null;
      ...
      `SELECT
         ...
         ${selectExpression("source_kind", "'official'")},
         ${selectExpression("official_catalog_active", "1")},
    ...
    this.transaction(() => {
      ...
      this.run(
        `CREATE TABLE hololive_music_videos_next (
          ...
          source_kind TEXT NOT NULL DEFAULT 'official' CHECK(source_kind IN ('official', 'user')),
          official_catalog_active INTEGER NOT NULL DEFAULT 1 CHECK(official_catalog_active IN (0, 1)),
          source_run_id TEXT REFERENCES hololive_music_refresh_runs(id),
          updated_at TEXT NOT NULL
        )`
      );
      ...
      for (const row of rows) {
        ...
        `INSERT INTO hololive_music_videos_next
          ...
          [
            ...
            row.source_kind === "user" ? "user" : "official",
            row.official_catalog_active === 0 ? 0 : 1,
      ...
      this.run("CREATE INDEX IF NOT EXISTS idx_hololive_music_videos_source_kind ON hololive_music_videos(source_kind)");
      this.run(
        "CREATE INDEX IF NOT EXISTS idx_hololive_music_videos_catalog_active ON hololive_music_videos(official_catalog_active, source_kind)"
      );
  ...
  private deleteExcludedHolodexChannelData(): void {
    ...
    this.run(
      `DELETE FROM hololive_music_detail_cache
       WHERE (channel_id IN (${placeholders}) OR original_channel_id IN (${placeholders}))
  ...
  private backfillHololiveMusicClassifications(): void {
    ...
    const updateClassifications = () => {
      for (const row of rows) {
        ...
        const participants = this.buildHololiveMusicParticipants(
          detail,
          channelContext,
          row.topic_id,
          {
        ...
        if (
          (row.participants_json || "[]") === participantsJson &&
  ...
  private backfillHololiveMusicSourceKinds(): void {
    if (!this.tableExists("hololive_music_videos") || !this.tableColumnNames("hololive_music_videos").has("source_kind")) {
      return;
    }
