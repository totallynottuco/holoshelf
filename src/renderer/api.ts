...
import type { HoloshelfBridge, IpcChannel, IpcChannelMap, UpdateStatus } from "../shared/ipc";
...
const mockHololiveMusicExclusions: Record<string, IpcChannelMap["hololive:music:exclude"]["response"]> = {};
...
function getMockHololiveBracketStatsOverview(): HololiveBracketStatsOverview {
  ...
  return {
    ...
    topTalents: [...talentStats.values()].sort(byWins).slice(0, 10),
...
export const api: HoloshelfBridge = window.holoshelf ?? (canUseBrowserMock() ? mockBridge : missingBridge);
